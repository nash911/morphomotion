...
/*                                                                                                              */
/*   I N V E R S E S I N E   C O N T R O L L E R   V 2   C L A S S                                              */
/*                                                                                                              */
/*   Avinash Ranganath                                                                                          */
...
InverseSineController_V2::InverseSineController_V2(void):Controller()
{
}


InverseSineController_V2::InverseSineController_V2(Flood::MultilayerPerceptron* mlp_pointer, Robot* pointer_robot_primary):Controller(mlp_pointer, pointer_robot_primary)
{
}


InverseSineController_V2::InverseSineController_V2(Flood::MultilayerPerceptron* mlp_pointer, Robot* pointer_robot_primary, Robot* pointer_robot_secondary):Controller(mlp_pointer, pointer_robot_primary, pointer_robot_secondary)
{
}


void InverseSineController_V2::init_controller(const double delta_time)
{
...
void InverseSineController_V2::set_default(void)
{
  Controller::set_default();

  oscillator_amplitude = 0;
  oscillator_offset = 0;
  servo_delta_threshold = 0.0;
}


void InverseSineController_V2::init_local_variables(Flood::Vector<double> &output,
...
void InverseSineController_V2::start_Controller(const std::string& type, std::stringstream& SS, int generation)
{
  //--Reset controller.
  reset_controller();
  ...
  std::thread ctrl(&InverseSineController_V2::run_Controller, this, type, std::ref(SS), generation);
  std::thread read_broadcast(&InverseSineController_V2::read_servo_positions_with_time_THREAD, this);
...
void InverseSineController_V2::run_Controller(const std::string& type, std::stringstream& SS, int generation)
{
  set_servo_delta_threshold(mlp->get_independent_parameter(0));
  set_oscillator_amplitude(mlp->get_independent_parameter(1));
  set_oscillator_offset(mlp->get_independent_parameter(2));
  set_oscillator_frequency(mlp->get_independent_parameter(3));
  ...
  do
  {
      do
      {
        ...
        for(unsigned int module=0; module<number_of_modules; module++)
        {
          ...
          if(actual_diff > servo_delta_threshold)
          {
...
void InverseSineController_V2::actuate_with_inverse_sine_controller(const unsigned int module, const double tPrime, Flood::Vector<double>& output)
{
...
void InverseSineController_V2::update_tPrime(unsigned int module)
{
...
double InverseSineController_V2::calculate_tPrime(double Theta, double V)
{
...
double InverseSineController_V2::inverse_sinewave_function(const double Theta, const double V)
{
  //--               |‾                                                              ‾|
...
double InverseSineController_V2::getCurrentVelocityDirection(double current_servo_position)
{
...
double InverseSineController_V2::get_t(const unsigned int module)
{
  double t;
